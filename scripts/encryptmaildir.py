...
def log(msg,infotype="m",ln=-1):
	...
	if LOGGING:
		...
		_lntxt="Line %i: "%ln
		tm=("%02d.%02d.%04d %02d:%02d:%02d:" %
			(t[2],t[1],t[0],t[3],t[4],t[5])).ljust(_lftmsg)
...
def get_lock(directory):
	...
	try:
		...
		process = subprocess.Popen([MAILDIRLOCK,directory,'10'],
...
class maildirname:
		...
		def __splitname(self,n):
			...
			try:
				...
				log("Name '%s' does not fit name convention "
					"part1.part2.part3:2,<flags>"%n,
					"e",
					lineno())
		...
		def set_dovecotflags(self,df=()):
			...
			if type(df)==str:
				...
				if len(r)!=2:
					log("Flag '%s' does not fit the convention name=value"%df,
						"w",
						lineno())
				else:
					self._dovecotflags[r[0]]=r[1]
				...
				for f in df:
					...
					if len(r)!=2:
						print("Flag '%s' does not fit the convention "
								"name=value"%f)
					else:
						self._dovecotflags[r[0]]=r[1]
...
def gzip_file(fname):
	...
	try:
		...
		log("gzip_file: File attributes for file '%s' couldn't be read"%fname,
			"e",
			ln=lineno())
		return False
	...
	if _res:
		...
		try:
			...
			log("gzip_file: Fileflags of file '%s' couldn't be set"%_newname,
				"w",
				ln=lineno())
		...
		try:
			...
			log("gzip_file: Error moving temporary file from "
			"'%(NEW)s' to '%(OLD)s'"%{	"NEW":_newname,
										"OLD":fname},
										"e",lineno())
			log("'%(m1)s %(m2)s'"%{	"m1":sys.exc_info()[0],
									"m2":sys.exc_info()[1]},
									"e",lineno())
			_res=False
...
def gunzip_file(fname):
	...
	try:
		...
		log("gunzip_file: Couldn't load file attributes of file '%s'"%fname,
													"e",
													lineno())
		return False
	...
	if _res:
		...
		try:
			...
			log("gzip_file: Couldn't set file flags of file '%s'"%_newname,
				"w",
				lineno())
		...
		try:
			...
			log("gzip_file: Couldn't move temporary file '%(NEW)s' to"
			" '%(OLD)s'"%{"NEW":_newname,"OLD":fname},"e",lineno())
			log("'%(m1)s %(m2)s'"%{	"m1":sys.exc_info()[0],
									"m2":sys.exc_info()[1]},"e",lineno())
			return False
		...
		try:
			...
			log("gunzip_file: Couldn't delete temporary file '%s'"%_newname,
				"w",lineno())
...
def bz2zip_file(fname):
	...
	try:
		...
		log("bz2zip_file: File attributes for file '%s' couldn't be read"%fname,
			"e",ln=lineno())
		return False
	...
	if _res:
		...
		try:
			...
			log("bz2zip_file: Fileflags of file '%s' couldn't be set"%_newname,
				"w",ln=lineno())

		try:
			shutil.move(_newname,fname)
		except:
			log("bz2zip_file: Error moving temporary file from"
				" '%(NEW)s' to '%(OLD)s'"%{"NEW":_newname,"OLD":fname},
				"e",lineno())
			_res=False
...
def bz2unzip_file(fname):
	...
	try:
		...
		log("bz2unzip_file: Couldn't load file attributes of file '%s'"%fname,
		"e",lineno())
		return False
	...
	if _res:
		...
		try:
			...
			log("bz2zip_file: Couldn't set file flags of file '%s'"%_newname,
			"w",lineno())

		try:
			shutil.move(_newname,fname)
		except:
			log("bz2zip_file: Couldn't move temporary file '%(NEW)s' to"
			" '%(OLD)s'"%{"NEW":_newname,"OLD":fname},"e",lineno())
			return False
		...
		try:
			...
			log("bz2unzip_file: Couldn't delete temporary file '%s'"%_newname,
			"w",lineno())
...
def do_finally():
	...
	if checked_files >0:
		...
		log("User %s: %i mail%s checked for encryption"%(	USER,
															checked_files,
															plural))
...
def parse_commandline():
	global to_addrs
	global CONFIGFILE,DEBUG,DOVECOT,DIRECTORIES,KEYHOME
	global USER,LOGFILE,LOGGING,SYSLOG

	try:
		cl=sys.argv[1:]
		_opts,_remainder=getopt.gnu_getopt(cl,'c:dhk:n:u:vxy',['config=',
																'debug',
																'dovecot',
																'example',
																'help',
...
def encrypt_dir(arg,dirname,names):
	...
	for n in names:
		...
		if ((DOVECOT and "dovecot" in n) or (MTIMEFILE in n)
				or("subscriptions" in n) or ("courierimap" in n)):
			debug("Ignore '%s'"%n)
			continue
		...
		if os.path.isfile(f) and filetime >mtime:
			...
			out=tempfile.NamedTemporaryFile(mode='wb',
											...
											prefix='mail-')
			out.close()
			cmd="%(CMD)s -l syslog  -a -k %(KEY)s -f '%(IN)s'"
			" -m %(OUT)s %(USER)s" %{	"CMD":GPGMAILENCRYPT,
										"KEY":KEYHOME,
										"IN":f,
										"OUT":out.name,
										"USER":USER}
...
SYSLOG=False
KEYHOME="~/.gnupg"
GPGMAILENCRYPT=shutil.which("gme.py")
...
try:
	...
	log("Keyboard: CTRL-C Exit ...",ln=lineno())
  	...
  	log("Bug: Exception '%(m1)s %(m2)s' occured!"%{	"m1":sys.exc_info()[0],
  													"m2":sys.exc_info()[1]})
